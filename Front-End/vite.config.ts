...
/**
 * Serves src/imports/*.json as /fixtures/*.json so fixture mode works
 * without needing to copy large files into public/.
 */
function serveImportsAsFixtures(): Plugin {
  const importsDir = path.resolve(__dirname, 'src/imports')
  return {
    name: 'serve-imports-as-fixtures',
    apply: 'serve',
    configureServer(server) {
      server.middlewares.use((req, res, next) => {
        const url = (req.url || '').split('?')[0]
        if (!url.startsWith('/fixtures/')) return next()
        const fileName = url.slice('/fixtures/'.length)
